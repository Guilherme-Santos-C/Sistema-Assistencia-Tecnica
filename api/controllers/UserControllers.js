...
const UserControllers = {
    create: async (req, res) => {
        try {
            ...
            let admin_senha_check = false
            if (Admin.password == req.body.password_admin) {
                admin_senha_check = true
            }
            else {
                try {
                    const verificacao = await bcrypt.compare(req.body.password_admin, Admin.password)
                    if (verificacao) {
                        admin_senha_check = true
                    }
                }
            ...
            if(!admin_senha_check) return res.status(400).json({ mensagem: "Senha do admin inválida!" })
    ...
    delete: async (req, res) => {
        try {
            // id vem na url
            const resposta_db = await UserModel.findByIdAndDelete(req.query.id)
    ...
    procurar: async (req, res) => {
        try {
            ...
            if (Admin.password != "Admin") {
                try {
                    await bcrypt.compare(req.query.admin, Admin.password)
                } catch (err) {
                    return res.json({ mensagem: "Senha do admin inválida" }).status(400)
                }
            } else if (Admin.password != req.body.password_admin) return res.status(400).json({ mensagem: "Senha do admin inválida" })
